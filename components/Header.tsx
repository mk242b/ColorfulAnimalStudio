...
export const Header: React.FC = () => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-white/95 backdrop-blur-md shadow-md py-2' : 'bg-transparent py-4 md:py-6'}`}>
      <div className="container mx-auto px-4 flex justify-between items-center">
        ...
        <a href="#" className="flex items-center gap-2 group">
          <img 
            ...
            className={`transition-all duration-300 ${isScrolled ? 'h-10 md:h-16' : 'h-12 md:h-20'} w-auto object-contain drop-shadow-sm group-hover:scale-105`} 
          />
          {/* Text fallback - Hidden on mobile, visible on large screens */}
          <span className={`font-black tracking-tighter text-gray-800 leading-none ${isScrolled ? 'text-lg' : 'text-xl'} hidden xl:block`}>
        ...
        <div className="md:hidden flex items-center gap-3">
          <button 
            onClick={toggleLanguage}
            className="px-2 py-1 rounded-full bg-brand-blue/10 text-brand-blue font-bold text-[10px] border border-brand-blue/20"
          >
            {language === 'en' ? 'EN' : 'MM'}
          </button>
          
          <button 
            className="p-1 text-gray-700 focus:outline-none active:scale-95 transition-transform"
